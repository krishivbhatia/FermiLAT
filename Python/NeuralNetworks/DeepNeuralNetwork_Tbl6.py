...
class DeepNeuralNetworkModel_Tbl6(nn.Module):
    def __init__(self, input_features):
        print("(len(input_features[0][0]) = ", len(input_features[0][0]))
        print("input_features[0][0] = ", input_features[0][0])
        super(DeepNeuralNetworkModel_Tbl6, self).__init__()
        self.model = nn.Sequential(nn.Linear(len(input_features[0][0]), 75),
                                   nn.ReLU(),
                                   nn.Linear(75, 25),
                                   nn.ReLU(),
                                   nn.Linear(25, 1))
