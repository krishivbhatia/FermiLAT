...
random_forest = TorchRandomForestClassifier(500, 500, 10)
train_features = ([((i[0]).detach().numpy()) for i in train_dataset])
train_labels = ([torch.IntTensor.item(i[1]) for i in train_dataset])
random_forest.fit(torch.FloatTensor(train_features), torch.IntTensor(train_labels))
...
print("correct/test_size = ", correct*100/test_size)